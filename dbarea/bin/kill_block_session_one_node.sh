...
if [ $# -ne 1 ]; then
    DB_NAME_LIST=`ps aux | grep ora_smon | grep -v grep | awk '{print $NF}' | awk -F '_' '{print $NF}' | grep -i -E "RACBTW2|WEB" | sed 's/.$//' | tr '[a-z]' '[A-Z]'`
else
    ORACLE_SID_LIST=($1)
...
if [ "x${emailcontent}" != "x" ]; then
    ...
    echo "The lock holder session under TEST or Shareplex user will be killed automatically. " >> ${maillogfile}
    echo "<table border=\"1px\"><tr><td>Session Role</td><td>inst_id</td><td>spid</td><td>sid</td><td>serial#</td><td>block time</td><td>osuser</td><td>Schema name</td><td>Program</td><td>Event</td><td>Machine</td><td>SQL_ID</td><td>LOCK_ID1</td><td>LOCK_ID2</td><td>LOCK_TYPE</td><td>BIND_VALUE</td></tr>${emailcontent}</table>" >> ${maillogfile}
    echo -e "\n\n\n<br>" >> ${maillogfile}
    echo "Below is the SQL list that holder or waiter is executing:" >> ${maillogfile}
