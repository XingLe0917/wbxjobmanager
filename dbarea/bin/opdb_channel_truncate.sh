...
$ORACLE_HOME/bin/sqlplus -S $ORACLE_USER/$ORACLE_PASSWORD@$ORACLE_SID >$TEMP_HOME/partition_drop.sql<< EOF
  ...
  where ( table_name like 'OPDB%' or table_name like 'WBXEVENTLOG' or table_name like 'WBXMEETINGLOGPEDP%' or table_name like 'WBXMEETINGLOG_DIST') 
  and   num_rows > 0
          ...
          trim (
             '''' from regexp_substr (
                          extractvalue (
                             ...
                             '//text()'),
                          '''.*?''')),
          'syyyy-mm-dd hh24:mi:ss') <  sysdate-30
