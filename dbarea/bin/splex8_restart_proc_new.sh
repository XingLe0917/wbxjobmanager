...
echo "list param all capture " | ./sp_ctrl |grep -v "^sp_ctrl " |grep -v "^Process " |grep -v "^--------" |grep -v "^$" > $TMPFILE4
...
then
 ...
 cat $TMPFILE4  |grep -i ddl |grep -vi debug |grep -vi logging > $TMPFILE5
