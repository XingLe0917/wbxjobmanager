...
if [ $? -eq 0 ]; then
    SPPORT="${GRANULARITY}"
    SQL=" select distinct src_db||','||src_splex_sid||','||port||','||di.application_type||','||di.appln_support_code 
          from shareplex_info si, database_info di
          where si.port=${SPPORT}
          and si.src_db=di.db_name
          and si.src_host like di.trim_host||'%'
          and si.src_host in "
else
    DB_NAME="${GRANULARITY}"
    SQL=" select distinct src_db||','||src_splex_sid||','||port||','||di.application_type||','||di.appln_support_code 
...
compareconfigfile()
{
    ...
    if [ $? -eq 0 ]; then
        SPPORT="${GRANULARITY}"
        SQL="select distinct si.src_splex_sid||','||si.tgt_splex_sid||','||si.src_schema||','||upper(srcai.appln_support_code)||','||upper(tgtai.appln_support_code)
from shareplex_info si, appln_pool_info srcai,appln_pool_info tgtai
where si.port=${SPPORT} 
and si.src_db=srcai.db_name
and si.src_schema=srcai.schema
and si.src_host like srcai.trim_host||'%'
and si.tgt_db=tgtai.db_name
and si.tgt_schema=tgtai.schema
and si.tgt_host like tgtai.trim_host||'%'
...
from (
    (select releasenumber,'${src_appln_support_code}' as src_appln_support_code, src_tablename,'${tgt_appln_support_code}' as tgt_appln_support_code,
           tgt_tablename, specifiedkey,columnfilter, specifiedcolumn,'ADDED' tablestatus,'DB' as datasrc
     ...
     (select releasenumber, src_appln_support_code, src_tablename,tgt_appln_support_code, tgt_tablename,
      ...
      minus
      select releasenumber,'${src_appln_support_code}' as src_appln_support_code, src_tablename,'${tgt_appln_support_code}' as tgt_appln_support_code,
            tgt_tablename, specifiedkey,columnfilter, specifiedcolumn,'ADDED' tablestatus,'baseline' as datasrc
      from wbxshareplextable 
      where src_splex_sid='${src_splex_sid}' 
      and tgt_splex_sid='${tgt_splex_sid}')
