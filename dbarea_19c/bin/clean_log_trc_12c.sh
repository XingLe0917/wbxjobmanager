...
GRID_HOME="`ps -ef | grep -v grep | grep crsd\.bin | awk '{print $8}' | awk -F'/' '{for(x=1;x<=NF-2;x++){printf("/%s", $x);} print "\n"; }' | sed -e 's/\/\//\//g'`"
Log "_crs_home=${GRID_HOME}"
...
myEcho "DEST_DIR=${DEST_DIR}"
  ...
  /usr/bin/find ${GRID_HOME}/rdbms/log/ -name "*.trc" -mtime $DAYS -exec rm -rf {} \;
...
/usr/bin/find ${GRID_HOME}/log/${HOSTNAME_S}/client/ -name "*.log" -mtime $DAYS -exec rm -rf {} \;

# remove scan listener old logs
if [ -n "`ls ${DEST_DIR}/tnslsnr/${HOSTNAME_S}/listener_scan*/alert 2>/dev/null`" ]; then
    /usr/bin/find ${DEST_DIR}/tnslsnr/${HOSTNAME_S}/listener_scan*/alert/ -name "log_*.xml" -mtime $DAYS -exec rm -rf {} \;
fi
##########################################################################
# call STAP API.
##########################################################################
